...
func (s *server) handleMovieDetail() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			log.Printf("Cannot parse id to int movies. err=%v", err)
			s.respond(w, r, nil, http.StatusBadRequest)
			return
		}
		m, err := s.store.GetMovieById(id)
		if err != nil {
			log.Printf("Cannot load movie with id=%v. err=%v", id, err)
			s.respond(w, r, nil, http.StatusInternalServerError)
			return
		}
...
func (s *server) handleMovieCreate() http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			log.Printf("Cannot not parse movie body. err=%v", err)
			s.respond(w, r, nil, http.StatusBadRequest)
		}
		m := &Movie{
			ID: 0,
		...
		if err != nil {
			log.Printf("Cannot create movie in databse. err=%v", err)
			s.respond(w, r, nil, http.StatusInternalServerError)
		}
		var resp = mapMovieToJson(m)
